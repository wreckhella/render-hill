<?php
session_name("BRICK-SESSION"); //да простит меня Isaiah...
session_start();
...
include('../../core/PHP/helper.php');
  ...
  if(isset($_POST['img'])){
	...
	if(base64_decode($image64)) {
		...
		$ID = $id;
		$imageFile = fopen("../../images/avatars/$ID.png", "w") or die("Unable to write avatar!");
		$data = $img;
		if ($imageFile) {
			...
			if ($writeImage) {
				...
				?> <script> window.location = "/images/avatars/<?php echo $ID; ?>.png?c=<?php echo rand() ?>"; </script>
				<?php
				die();
