...
include('../../../core/PHP/helper.php');
  ...
  if(isset($_POST['img'])){
	...
	if(base64_decode($image64)) {
		...
		$ID = $id;
		$imageFile = fopen("../../../images/headshots/$ID.png", "w") or die("Unable to write avatar!");
		$data = $img;
		if ($imageFile) {
			...
			if ($writeImage) {
				...
				?> <script> window.location = "/images/headshots/<?php echo $ID; ?>.png?c=<?php echo rand() ?>"; </script>
				<?php
				die();
  ...
  <body>
    ...
    <script>
      ...
      function init() {
        ...
        //camera.position.set(0,2.25,2.25);
    ...
    scene.add( directionalLight );
        ...
        loader.load( '/shop/assets/faces/<?php if ($avatar->{"face"} > 0) { echo shopItemHash($avatar->{"face"}); } else { echo "face"; }; ?>.png' , function ( faceimg ) {
          facetex.image = faceimg;
          facetex.needsUpdate = true;
        } );
        loader.load( '/shop/assets/tshirts/<?php if ($avatar->{"tshirt"} > 0) { echo shopItemHash($avatar->{"tshirt"});}; ?>.png' , function ( tshirtimg ) {
          tshirttex.image = tshirtimg;
          tshirttex.needsUpdate = true;
        } );
        loader.load( '/shop/assets/shirts/<?php if ($avatar->{"shirt"} > 0) { echo shopItemHash($avatar->{"shirt"}); } else { echo "studs"; }; ?>.png' , function ( shirtimg ) {
          shirttex.image = shirtimg;
          shirttex.needsUpdate = true;
        } );
        loader.load( '/shop/assets/pants/<?php if ($avatar->{"pants"} > 0) { echo shopItemHash($avatar->{"pants"}); } else { echo "studs"; }; ?>.png' , function ( pantsimg ) {
        ...
        loader.load( '/shop/assets/tools/<?php echo shopItemHash($avatar->{"tool"}); ?>.png' , function ( toolimg ) {
          tooltex.image = toolimg;
          tooltex.needsUpdate = true;
        } );
        // model
        var loader = new THREE.OBJLoader( manager );
        loader.load( '<?php if($avatar->{"head"} > 0) { echo "/shop/assets/heads/"; ?><?php echo $avatar->{"head"}; ?><?php echo ".obj" ;} else { echo "Head.obj"; } ?>', function ( head ) {
...
//uv map of it
        ...
        */
        ...
        if ($avatar->{"tool"} > 0) {
        // echo tool
			echo "loader.load( '/shop/assets/tools/"?><?php echo shopItemHash($avatar->{'tool'});?><?php echo ".obj', function ( tool ) {";
			echo "tool.traverse( function ( child ) {";
			echo "if ( child instanceof THREE.Mesh ) {";
			echo "child.material.map = tooltex;";
			echo "}";
			echo "} );";
			echo "tool.position.y = -2.5;";
			...
			echo "}";
			echo "} );";
			echo "leftArmUpColor.position.y = -2.5;";
