...
if(isset($_GET['page'])) {
	...
	if(isset($_GET['item']) || isset($_GET['search'])) {
		...
		$crateSQL = $crateSQL."`approved`='yes' ORDER BY `last_updated` DESC LIMIT $limit,36";
	} else {
		$crateSQL = "SELECT * FROM `shop_items` WHERE `type`='hat' OR `type`='tool' OR `type`='face' ORDER BY `last_updated` DESC LIMIT $limit,36";
	...
	while($searchRow=$result->fetch_assoc()){
		...
		//if($r >= ($page*36)) {
			...
			echo 'id="shopItem" src="/shop/thumbnails/' . $thumbnail . '.png?c=' . rand() . '"></a>'; 
			...
			echo '<span style="display:inline-block; float:left; width:100%">
				...
				<span style="display:inline-block; float:left; padding-left:0px; color: #c3cdd2;"><p class="shopDesc">by<a class="shopDesc" href="/user?id='. $searchRow['owner_id'] . '">' . $userRow['username'] . '</a></p></span>
