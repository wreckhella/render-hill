...
if($loggedIn) {
  ...
  if (isset($_GET['scrub_comment']) && $currentUserRow['power'] >= 5) {
...
<!DOCTYPE html>
  <head>
    <title><?php echo htmlentities($shopRow['name']); ?></title>
  <meta property="og:title" content="<?php echo $shopRow['name']; ?> - thedenbruh deez nuts hell item" />
  <meta property="og:description" content="<?php echo htmlentities($shopRow['description']); ?>" />
  <meta property="og:image" content="<?php echo '/shop/thumbnails/'; ?><?php echo $shopRow['id'];?><?php echo ".png?c=";?>" />
  <meta property="og:type" content="website" />
  <meta property="og:url" content="/item?id=<?php echo $shopRow['id'] ?>" />
  </head>
  <body>
    <div id="body">
      ...
      echo '</h3>';
        ...
        <span style="display:inline-block; float:left; width:640px;">
          ...
          if(isset($_GET['render']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/customize/rendering/shop_render?id='.$shopRow['id'].'"></iframe>';
          }
	if(isset($_GET['hatrender']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/customize/rendering/item/hat?id='.$shopRow['id'].'"></iframe>';
          }
	if(isset($_GET['toolrender']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/customize/rendering/item/tool?id='.$shopRow['id'].'"></iframe>';
          }
	if(isset($_GET['headrender']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/customize/rendering/item/head?id='.$shopRow['id'].'"></iframe>';
          }
if(isset($_GET['trender']) && ($shopRow['owner_id'] == $_SESSION['id'] || $currentUserRow['power'] >= 1)) {
            echo '<iframe style="background-color:#FFF;float:left;border:1px #c3cdd2 solid;width:340px;height:340px;" src="/customize/rendering/item/tool?id='.$shopRow['id'].'"></iframe>';
          }
          else {
            echo '<img id="shopItem" src="/shop/thumbnails/'.$thumbnail.'.png?c='. rand() .'" style="float:left; margin: 0px 10px 0px 10px; width: 340px; height: 340px;';
      ...
      if ($shopRow['bucks'] == 0 || $shopRow['bits'] == 0) { 
        echo "<span style='display:inline-block;'>
          ...
          <form method='post' action=''>
            <input style='border:1px solid #c3cdd2; background-color:#44A4EE; color:#FFF;'' type='submit' name='buyFree' value='Take one'>
          </form>
        </span>";
      }
      ...
      if ($shopRow['bucks'] >= 1) {
        echo "<span style='display:inline-block;''>
          ...
          <br>
          <form method='post' action=''>
            <input style='border:1px solid #c3cdd2; background-color:Green; color:#FFF;' type='submit' name='buyBucks' value='Buy with Bucks'>
          </form>
        </span>";
      }
      ...
      if ($shopRow['bits'] >= 1) {
        echo "<span style='display:inline-block;''><a style='color:Orange;''><i class='fa fa-circle' style='padding-right: 5px;'></i>$shopRow[bits]</a>
          <br>
          <form method='post' action=''>
            <input style='border:1px solid #c3cdd2; background-color:Orange; color:#FFF;' type='submit' name='buyBits' value='Buy with Bits'>
          </form>
        </span>";
      }
    }
    ...
    ?>
        ...
        <span style="display:inline-block; float:right;">
          <img style="width:225px; background-color:transparent;" src="/images/avatars/<?php echo $userRow['id']; ?>.png?c=<?php echo $userRow['avatar_id']; ?>">
          <h5>Created by:</h5>
          <a href="/user?id=<?php echo $userRow['id']; ?>" style="color:#222;"><h4><?php echo $userRow['username']; ?></h4></a>
        </span>
      </div>
      <?php
        if($remaining <= 0) {
      ...
      if($sellRow['active'] == 'yes') {
        ...
        <div id="subsect" style="overflow:auto;">
          ...
          <div style="width:100px;float:left;">
            <a href="/user?id='.$sellerID.'"><img style="width:70px;" src="/images/avatars/'.$sellerID.'.png?c='.$sellerRow['avatar_id'].'"></a>
...
<?php } ?>
        ...
        while($commentRow = mysqli_fetch_assoc($comments)) {
              ...
              echo'<div id="subsect" style="overflow:auto;"><span style="float:left; color: #c3cdd2;"><a href="/user?id='.$commentUserID.'" style="color:#c3cdd2;"><img style="width:55px;" src="/images/avatars/'.$commentUserID.'.png?c='.rand().'"><br>'.$userData['username'].'</a></span><span style="color: #c3cdd2;"><span style="font-size:12px; color: #c3cdd2;">'.$commentRow['time'].'</span><br>'. htmlentities($commentRow['comment']).'</span>';
              if($loggedIn && $power >= 5) {
                echo '<div><a class="label" style="color: #c3cdd2;" href="item?id='.$shopRow['id'].'&scrub_comment='.$commentRow['id'].'">Scrub</a></div>';
              }
              echo '</div>';
        }
        ?>
