...
if($power < 0) { header('Location: ../');die(); }
		...
		if($isImage !== false) {
			if($imgSize < 2097152) {
				if(isset($_POST['name'])) {
						...
						$newSQL = "INSERT INTO `shop_items` (`id`,`owner_id`,`name`,`description`,`bucks`,`bits`,`type`,`date`,`last_updated`,`offsale`,`collectible`,`collectable-edition`,`collectible_q`,`zoom`,`approved`) VALUES (NULL,'$ownerID','$name','$desc','$bucks','$bits','face','$today','$today','no','no','no','0',NULL,'yes')";
						$new = $conn->query($newSQL);
						$itemID = $conn->insert_id;
						
						move_uploaded_file($imgTmp,"../assets/faces/".shopItemHash($itemID).".png");
						header('Location: ../item?id='.$itemID.'&render');
					} else {
...
<!DOCTYPE html>
	<head>
		<title>Upload Face - Render Hill</title>
	</head>
	<body>
		<div id="body">
			<div id="box">
				<?php
				if(!empty($error)) {
					echo '<div style="background-color:#EEc3cdd23;margin:10px;padding:5px;color:white;">';
