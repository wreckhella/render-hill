...
<!DOCTYPE html>
  <head>
		<?php /* if($_SERVER['REQUEST_URI'] != '/user.php?='.$id.'') { echo '
		<script type="text/javascript">location.href = "/user?id='.$id.'";</script>';
		} else {
	//Do nonthing.
		} */
		?>
    <title><?php echo $userRow['username']; ?> - Render Hill</title>
  <meta charset="UTF-8">
  <meta name="description" content="<?php echo $userRow['username'] ?> - an user">
  <meta name="keywords" content="free,game">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  
  <meta property="og:title" content="<?php echo $userRow['username']; ?> - Render Hill User" />
  <meta property="og:description" content="<?php echo $userRow['description']; ?>" />
  <meta property="og:image" content="<?php echo '/images/avatars/'; ?><?php echo $userRow['id'];?><?php echo ".png?c=";?><?php echo $userRow['avatar_id']; ?>" />
    ...
    ?>
      ...
      <div id="column" style="width:394px; float:left;">
		...
		if($power >= 5) {
          ...
          <img src="/images/avatars/<?php echo $userRow['id']; ?>.png?c=<?php echo $userRow['avatar_id']; ?>" style="width: 235px;height: 280px;border:0px;">
		  <?php } ?>
          <div style="padding:15px;word-break:break-word; color: #c3cdd2;"><br><p><?php if (!empty($userRow['description']) || $userRow['description'] !== NULL ) {echo nl2br(htmlentities($userRow['description']));}
...
if($loggedIn) {
  	    ...
  	    $AlreadyFriends = mysqli_num_rows($AlreadyFriendsQ);
            ...
            if($AlreadyFriends<1){
              ...
              echo '<a href="/friends/remove?id=' . $id . '"><input class="red-button" type="button" value="Unfriend"></a>';
            }
          
          } else {
          echo '<a href="/customize"><input type="button" value="Customize"></a><a style="padding-left:5px;" href="/settings"><input type="button" value="Settings"></a>';
          }
        if($power >= 5 && $userRow['power'] < $power) {
        	echo '<br><a href="/ban?id='.$id.'">
        	<input class="red-button" type="button" value="Ban User">
        	</a>
        	<a href="/customize/rendering/?id='.$id.'">
        ...
        <div id="box" style="width:100%; margin-top:10px; text-align:center;">
          ...
          if($rewards->num_rows != 0){
	          while($rewardsRow = $rewards->fetch_assoc()){
	          	...
	          	echo '<div style="margin: 10px;width: 107px;display:inline-block;float: left;"><a href="/information/awards/" style="color:#c3cdd2;">
	...
	while($membershipRow = $membership->fetch_assoc()) {
		...
		elseif ($findClanRow['approved'] == 'declined') {$thumbnail = 'declined';}
	          ...
	          <a href="/clan?id='.$clanID.'"><img class="profile-display" src="/images/clans/'.$thumbnail.'.png"></a>
	          	<span style="text-align:center;display:inline-block;float:left;width: 100%;padding-left: 0;padding-right: 0;">
      ...
      <div id="column" style="width:494px; float:right;">
        <div id="box" style="display: inline-block; width:100%; text-align:center;">
          ...
          while($gamesRow = $games->fetch_assoc()) {
          	echo '<div id="subsect" style="text-align:left;padding-right: 10px;padding-bottom: 10px;padding-left: 12px; color: #c3cdd2;">
            <a href="/play/set?id='.$gamesRow['id'].'"><h4 style="margin: 4px;">'.$gamesRow['name'].'</h4></a>
            <h6 style="margin: 4px;">'.$gamesRow['visits'].' Visits</h6>
            <a href="/play/set?id='.$gamesRow['id'].'"><img width="470px" src="/images/games/'.$gamesRow['id'].'.png"></a>
            <h5 style="margin: 4px; font-weight: normal;">'.$gamesRow['description'].'</h5>
          </div>';
          }
          if($games->num_rows == 0) {
          	echo '<em style="color: #c3cdd2;">This user has no games</em>';
          }
          ?>
        </div>
                  ...
                  echo "
                    <div id='friend'>
                        <div style='padding:2px;float: left;'>
                          <a style='color:#c3cdd2;text-decoration:none;' href='/user?id=".$friendRow['id']."' title='".$friendRow['username']."'>
...
<div id="column" style="width:494px; float:right;">
        <div id="box" style="display: inline-block; width:100%; text-align:center; margin-top:10px; color: #c3cdd2;">
          <div id="subsect">
