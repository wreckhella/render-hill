...
<!DOCTYPE html>
	<head>
		<title>Dashboard - TDBDNH: Revived</title>
	</head>
	<body>
		<div id="body">
			<div id="column" style="float:left; width:580px;">
				<div id="box" style="width:100%;">
				<?php if($loggedIn) {echo '<br><h2><center>Hello there, '. $userRow->{'username'}.'!</h2></center><br><hr>';} ?>
					<div style="margin:20px;float:left;position:relative;width: 165px;">
						<img style="width:180px" src="/images/headshots/<?php echo $userRow->{'id'}; ?>.png?c=<?php echo $userRow->{'avatar_id'}; ?>">
					</div>
				...
				<div id="box" style="margin-top:10px; width:100%;">
					...
					<div style="margin:10px;">
						...
						<img src="/images/avatars/<?php echo '1'; ?>.png?c=<?php echo $userRow->{'avatar_id'}; ?>" style="width:120px;">
						<a href="user?id=<?php echo '1';?>" class="title" style="font-weight:bold;"><?php echo 'den'; ?></a>
					</div>
				</div>
			</div>
...
for($c = 0; $c < 10; $c++) {
	...
	echo '<tr style="border-bottom: 1px solid #aaaaaa;">
		<td style="padding: 8px 0px 0px 8px;background: #bdbec1;" valign="top">'.str_replace(">","&gt;",str_replace("<","&lt;",$statusRow['body'])).'</td><td style="background: #bdbec1;font-size: 12.5px;font-style: italic;text-align: right;padding-top: 46px;float: right;padding-right: 5px;">'.$statusRow['date'].'</div>
...
/*while($statusRow = $status->fetch_assoc()) {
	...
	echo '<div id="subsect" style="overflow: auto;">
		...
		<a href="/user.php?id='.$statusRow['owner_id'].'" style="color:#000;text-decoration:none;">
			<img style="width:40px;" src="/images/avatars/'.$statusRow['owner_id'].'.png?c='.$posterRow['avatar_id'].'"><br>' .$posterRow['username']. '</a>
		</div>
